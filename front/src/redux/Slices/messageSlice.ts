...
export const messageSlice = createSlice({
  ...
  reducers: {
    ...
    filterUnreadMessages: (state, action: PayloadAction<string>) => {
      state.unreadMessages = state.unreadMessages.filter((m) => m.sender_id !== action.payload)
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addMatcher(isRejected(getMessages), (state, action) => {
        console.error('Messages receive sequence failed with error:', action.payload)
      })
      .addMatcher(isPending(getMessages), (state) => {
