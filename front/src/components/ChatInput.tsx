...
const ChatInput: FC<IProps> = ({ socket }) => {
  ...
  const { messageOnEdit, filesToDelete } = useAppSelector(
  ...
  useEffect(() => {
    if (messageOnEdit) {
      if (textInputRef.current)
        textInputRef.current.value = messageOnEdit.content;
    }
  }, [messageOnEdit]);
  const sendMessageHandle = async () => {
    ...
    socket?.emit("edit_message",{ content: textInputRef.current?.value || "",
      ...
      try {
        ...
        if (
          (textInputRef.current?.value.trim() || selectedFiles.length) &&
          socket
        ) {
          const messageData = {
  ...
  return (
    <div className="w-full h-[50px] gap-[5px] border border-gray-300 flex justify-content-end">
      <input
        disabled={!me_online}
        ref={textInputRef}
        type="text"
      ...
      <button
        ...
        className={
          "flex justify-center items-center w-[100px] flex-grow-0 h-full p-[5px]  hover:bg-gray-100 cursor-pointer"
        }
      >
